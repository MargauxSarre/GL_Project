...
describe('Restaurant e2e test', () => {
  ...
  it('should create and save Restaurants', async () => {
    ...
    await promise.all([
      ...
      restaurantUpdatePage.setAdresseInput('adresse'),
      restaurantUpdatePage.compteSelectLastOption(),
      // restaurantUpdatePage.typeCommerceSelectLastOption(),
    ]);

    expect(await restaurantUpdatePage.getNomInput()).to.eq('nom', 'Expected Nom value to be equals to nom');
    expect(await restaurantUpdatePage.getAdresseInput()).to.eq('adresse', 'Expected Adresse value to be equals to adresse');
