...
describe('Commande e2e test', () => {
  ...
  it('should create and save Commandes', async () => {
    ...
    await promise.all([
      commandeUpdatePage.setAdresseInput('adresse'),
      commandeUpdatePage.courseSelectLastOption(),
      commandeUpdatePage.compteSelectLastOption(),
      commandeUpdatePage.restaurantSelectLastOption(),
      // commandeUpdatePage.produitSelectLastOption(),
    ]);

    expect(await commandeUpdatePage.getAdresseInput()).to.eq('adresse', 'Expected Adresse value to be equals to adresse');
